...
export default function VitalsMonitor() {
  ...
  const [vitalsHistory, setVitalsHistory] = useState<Record<string, any[]>>({});
  const [selectedPatient, setSelectedPatient] = useState<any>(null);

  useEffect(() => {
    async function fetchPatients() {
      try {
        const res = await fetch(`${API}/admissions?status=active`, { headers: authHeader() });
        if (res.ok) {
  ...
  useEffect(() => {
    ...
    async function fetchTrend() {
      try {
        const res = await fetch(`${API}/vitals/${selectedPatient.patient_id}?limit=50`, { headers: authHeader() });
        if (res.ok) {
          const data = await res.json();
          // Map to match frontend component expected format
          const mappedHistory = (data.data || []).map((v: any) => ({
          ...
          setVitalsHistory(prev => ({ ...prev, [selectedPatient.id]: mappedHistory.reverse() }));
        }
      } catch (_) { }
    }
    fetchTrend();
    const interval = setInterval(fetchTrend, 30000); // Poll every 30s
          ...
          <>
            ...
            {vitalsHistory[selectedPatient.id] && vitalsHistory[selectedPatient.id].length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5 shrink-0">
                ...
                <VitalStatCard
                  title="Core Temp"
                  value={(vitalsHistory[selectedPatient.id][vitalsHistory[selectedPatient.id].length - 1].temperature || 0).toFixed(1)}
                  unit="°C"
...
function PatientCard({ patient, isActive, onClick, latestVitals }: { patient: any, isActive: boolean, onClick: () => void, latestVitals?: any }) {
