...
import { Users, Activity, AlertTriangle, BedDouble, ArrowRight, Wind, TrendingDown } from 'lucide-react';
         ...
         <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 bg-white border border-slate-200/60 rounded-3xl p-7 shadow-[0_8px_30px_rgb(0,0,0,0.02)] flex-1">
               <div className="flex justify-between items-center mb-6">
                  ...
                  <Link href="/alerts" className="text-sm font-semibold text-indigo-600 hover:text-indigo-700 bg-indigo-50 hover:bg-indigo-100 px-4 py-2 rounded-full transition-colors flex items-center gap-1">
               ...
               <div className="flex flex-col gap-3">
                  {alerts.filter(a => !a.resolved).slice(0, 5).map(alert => {
                     let tagLabel = 'L1: NURSE';
                     let tagStyle = 'bg-blue-100 text-blue-700 border-blue-200';
                     if (alert.type === 'Critical') {
                        tagLabel = 'L3: ICU ADMIN'; tagStyle = 'bg-rose-100 text-rose-700 border-rose-200';
                     }

                     const isTrend = alert.metric === 'TREND_ALERT'; // Backend dependent

                     return (
                     <Link href={`/patients/${alert.patientId}`} key={alert.id} className={cn(
                        "p-4 rounded-2xl border transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-4",
                        isTrend ? 'bg-gradient-to-r from-fuchsia-50/50 to-transparent border-fuchsia-200 shadow-sm' :
                        ...
                        <div className="flex items-center gap-4">
                           <div className={cn("p-2.5 rounded-xl shrink-0",
                              isTrend ? 'bg-fuchsia-100 text-fuchsia-600' :
                              alert.type === 'Critical' ? 'bg-rose-100 text-rose-600' : 'bg-orange-100 text-orange-600'
                           )}>
                              {isTrend ? <TrendingDown className="w-5 h-5 animate-pulse" /> : <AlertTriangle className="w-5 h-5" />}
                           </div>
                           <div className="min-w-0">
                              <p className="text-slate-800 font-bold truncate">
                                 {alert.patientName} 
                                 {isTrend ? <span className="font-bold text-fuchsia-600 text-xs ml-2 uppercase tracking-wide">Trend Warning</span> : null}
                              </p>
                              <p className="text-slate-600 text-sm mt-0.5 truncate">{alert.message}</p>
                           </div>
                        </div>
                        <div className="flex items-center gap-2 self-start sm:self-center">
                           <div className={cn("text-xs font-bold px-2.5 py-1 rounded-md border", tagStyle)}>
                              {tagLabel}
                           </div>
                           <div className="text-sm font-medium text-slate-400 bg-slate-50 px-3 py-1.5 rounded-lg whitespace-nowrap border border-slate-200/50">
            ...
            <div className="bg-gradient-to-br from-indigo-500 to-purple-600 rounded-3xl p-7 shadow-[0_8px_30px_rgba(99,102,241,0.2)] text-white flex flex-col justify-between overflow-hidden relative">
               ...
               <Link href="/settings" className="w-full mt-6 bg-white text-indigo-600 font-bold rounded-xl py-3 shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all relative z-10 block text-center">
                  System Settings
               </Link>
