...
export function Shell({ children }: { children: React.ReactNode }) {
  ...
  // Filter alerts for doctors vs admins vs patients
  const displayAlerts = alerts.filter(a => {
    if (currentUser?.role === 'patient') return false; // hide all alerts for patient
    // Could filter by doctor ID here if backend supported it, for now show all to doctors/admins
    return true;
  });
    ...
    <div className="flex h-screen bg-slate-50 text-slate-800 overflow-hidden font-sans selection:bg-indigo-500/20">
      ...
      <main className="flex-1 flex flex-col min-w-0 relative z-10">
        ...
        <div className="flex-1 overflow-auto scrollbar-hide relative bg-transparent p-4 md:p-6 lg:p-8">
          ...
          {/* Global Alerts Sidebar Overlay */}
          <AnimatePresence>
            {isAlertOpen && currentUser?.role !== 'patient' && (
              <>
                <motion.div
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
